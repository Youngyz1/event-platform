...
- DB schema lives in `db/` as sequential `migration_NN_*.sql` files (currently up to 67) plus feature schema files (`*_schema.sql`) and a consolidated `schema.sql`. Check the latest migration number before adding a new one.
- **PostgREST 1,000-row pagination limit**: Supabase JS (PostgREST) silently truncates unpaginated `.select()` queries at 1,000 rows by default. Any query expected to return >1,000 rows (auditing, exports, donor counts, batch aggregations) MUST use explicit `.range(start, end)` pagination loops (see `lib/donations.ts` or `app/api/admin/fundraisers/[id]/import/route.ts`).
